...
class mans():
    ...
    def reset(self):
        ...
        self.per_tick()
    def right_click(self,hand,mpos):
        #what happend when right muse button is clicked
        self.hand = hand
        in_box = False
        print(mpos[1],int(self.prop*700+self.delta_x),int(self.prop*801+self.delta_y))
        if mpos[1] in range(int(self.prop*700+self.delta_y),int(self.prop*801+self.delta_y)):
                if mpos[0] in range(int(self.prop*320+self.delta_x),int(self.prop*421+self.delta_x)):
                    self.hand = "Left Hand"
                    in_box = True
                if mpos[0] in range(int(self.prop*860+self.delta_x),int(self.prop*961+self.delta_x)):
        ...
        if in_box:
            if self.equipment[self.hand][0] == [None]:
                ...
                pygame.draw.polygon(self.screen,(150,150,150),(mpos,
                                                      (mpos[0]+size[0]+int(self.prop*20),mpos[1])
                                                      ,(mpos[0]+size[0]+int(self.prop*20),mpos[1]-(size[1]+int(self.prop*20))),
                                                      (mpos[0],mpos[1]-(size[1]+int(self.prop*20)))),0)
                pygame.draw.polygon(self.screen,(120,120,120),(mpos,
                                                      (mpos[0]+size[0]+int(self.prop*20),mpos[1])
                                                      ,(mpos[0]+size[0]+int(self.prop*20),mpos[1]-(size[1]+int(self.prop*20))),
                                                      (mpos[0],mpos[1]-(size[1]+int(self.prop*20)))),int(self.prop*5))
                self.screen.blit(texts,(mpos[0]+int(self.prop*10),mpos[1]-int(self.prop*10)-size[1]))
                self.options={ ((mpos[0],mpos[0]+size[0]+int(self.prop*20)),(mpos[1],mpos[1]-(size[1]+int(self.prop*20)))) :"Attack"}
        ...
        for i in list(self.options.keys()):
            ...
            if mpos[0] >= i[0][0]  and mpos[0] <= i[0][1] and mpos[1] >= i[1][1] and mpos[1] <= i[1][0]:
                ...
                if self.options[i] == "Attack":
                    self.attack(self.hand,who,"Head")
                else:
